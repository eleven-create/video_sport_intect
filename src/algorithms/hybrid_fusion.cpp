...
void HybridFusion::processFrame(const cv::Mat& frame, cv::Mat& fgMask) {
    ...
    if (prevGray.empty() || frameCount % 5 == 0 || prevPts.empty()) {
        // 在 MOG2 抠出的人影里找特征点
        cv::goodFeaturesToTrack(gray, prevPts, 100, 0.05, 10, rawMask);
    }
    ...
    if (!prevPts.empty() && !prevGray.empty()) {
        ...
        // ⭐ 优化2：将光流搜索窗口放大到 cv::Size(31, 31)，金字塔层数增至 3，极大提升抗快速运动能力！
        ...
        for (size_t i = 0; i < prevPts.size(); i++) {
            if (status[i]) {
                ...
                // ⭐ 优化3：只要光流认为动了（>1.0），就不再设置上限，充分信任光流！
                if (distance > 1.0) {
                    goodNewPts.push_back(nextPts[i]);
                    // 画一个半径为 35 的粗壮白色圆斑
                    cv::circle(flowMask, nextPts[i], 35, cv::Scalar(255), -1);
                }
            }
        }
        prevPts = goodNewPts; 
